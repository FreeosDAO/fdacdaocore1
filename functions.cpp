fdacdaocore1::groupconf fdacdaocore1::get_group_conf(){
  //groupconf conf;
  //return conf;
...
bool fdacdaocore1::is_member(const name& accountname){
...
bool fdacdaocore1::has_module(const name& module_name){
  modules_table _modules(get_self(), get_self().value);
  auto itr = _modules.find(module_name.value);
  return itr != _modules.end();
}

bool fdacdaocore1::member_has_balance(const name& accountname){
...
void fdacdaocore1::update_member_count(int delta){
...
void fdacdaocore1::update_custodian_count(int delta){
...
bool fdacdaocore1::is_account_voice_wrapper(const name& account){
  //if config voice only do stuff
  return is_account(account);
}

bool fdacdaocore1::is_custodian(const name& account, const bool& update_last_active, const bool& check_if_alive) {
...
void fdacdaocore1::update_custodian_last_active(const name& account){
  custodians_table _custodians(get_self(), get_self().value);
  auto cust_itr = _custodians.find(account.value);
...
bool fdacdaocore1::is_account_alive(time_point_sec last_active){
...
//TODO
void fdacdaocore1::update_custodian_weight(const name& account, const uint8_t& weight) {}
//TODO

void fdacdaocore1::update_owner_maintainance(const permission_level& maintainer){
...
void fdacdaocore1::update_active() {
...
void fdacdaocore1::sub_balance( const name& account, const extended_asset& value) {
...
void fdacdaocore1::add_balance( const name& account, const extended_asset& value){
  //uint128_t composite_id = (uint128_t{value.contract.value} << 64) | value.quantity.symbol.raw();
   balances_table _balances( get_self(), account.value);
   auto by_contr_sym = _balances.get_index<"bycontrsym"_n>(); 
   uint128_t composite_id = (uint128_t{value.contract.value} << 64) | value.quantity.symbol.raw();
...
bool fdacdaocore1::is_existing_threshold_name(const name& threshold_name){
...
uint8_t fdacdaocore1::get_threshold_by_name(const name& threshold_name){
  thresholds_table _thresholds(get_self(), get_self().value);
  auto thresh_itr = _thresholds.find(threshold_name.value);
  if(thresh_itr == _thresholds.end() ){
...
void fdacdaocore1::insert_or_update_or_delete_threshold(const name& threshold_name, const int8_t& threshold, const bool& remove, const bool& privileged){
   thresholds_table _thresholds(get_self(), get_self().value);
   auto thresh_itr = _thresholds.find(threshold_name.value);
...
bool fdacdaocore1::is_threshold_linked(const name& threshold_name){
...
//
fdacdaocore1::threshold_name_and_value fdacdaocore1::get_required_threshold_name_and_value_for_contract_action(const name& contract, const name& action_name){
...
uint8_t fdacdaocore1::get_total_approved_proposal_weight(proposals_table::const_iterator& prop_itr){
...
void fdacdaocore1::archive_proposal(const name& archive_type, proposals_table& idx, proposals_table::const_iterator& prop_itr){
