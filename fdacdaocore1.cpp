#include "fdacdaocore1.hpp"
#include "functions.cpp"

ACTION fdacdaocore1::updateconf(groupconf new_conf, bool remove){
...
///////////////////////////////////
ACTION fdacdaocore1::propose(name proposer, string title, string description, vector<action> actions, time_point_sec expiration) {
...
//////////////
ACTION fdacdaocore1::approve(name approver, uint64_t id) {
...
ACTION fdacdaocore1::unapprove(name unapprover, uint64_t id) {
...
ACTION fdacdaocore1::cancel(name canceler, uint64_t id) {
...
ACTION fdacdaocore1::exec(name executer, uint64_t id) {
...
ACTION fdacdaocore1::invitecust(name account){
  require_auth(get_self() );
...
ACTION fdacdaocore1::removecust(name account){
  require_auth(get_self());
...
ACTION fdacdaocore1::imalive(name account){
...
ACTION fdacdaocore1::isetcusts(vector<name> accounts){
...
ACTION fdacdaocore1::widthdraw(name account, extended_asset amount) {
  require_auth(account);
...
ACTION fdacdaocore1::internalxfr(name from, name to, extended_asset amount, string msg){
  require_auth(from);
  groupconf conf = get_group_conf();
...
ACTION fdacdaocore1::manthreshold(name threshold_name, int8_t threshold, bool remove){
  require_auth(get_self() );
  insert_or_update_or_delete_threshold(threshold_name, threshold, remove, false);//!!!!!!!!!!!!! false
}

ACTION fdacdaocore1::manthreshlin(name contract, name action_name, name threshold_name, bool remove){
  require_auth(get_self() );
...
ACTION fdacdaocore1::trunchistory( name archive_type, uint32_t batch_size){
  require_auth(get_self() );
...
ACTION fdacdaocore1::regmember(name actor){
  require_auth(actor);
  groupconf conf = get_group_conf();
...
ACTION fdacdaocore1::signuserterm(name member, bool agree_terms){
  require_auth(member);
  groupconf conf = get_group_conf();
...
ACTION fdacdaocore1::unregmember(name actor){
  require_auth(actor);
...
ACTION fdacdaocore1::updateavatar(name actor, string img_url){
  require_auth(actor);
...
ACTION fdacdaocore1::updatprofile(name actor, name key, string data){
  require_auth(actor);
...
ACTION fdacdaocore1::delprofile(name actor){
  require_auth(actor);
  profiledata_table _profiledata(get_self(), get_self().value);
...
ACTION fdacdaocore1::linkmodule(name module_name, permission_level slave_permission, bool has_contract){
  require_auth(get_self() );
  modules_table _modules(get_self(), get_self().value);
...
ACTION fdacdaocore1::unlinkmodule(name module_name){
  require_auth(get_self() );
  modules_table _modules(get_self(), get_self().value);
  auto itr = _modules.find(module_name.value);
...
ACTION fdacdaocore1::setuiframe(uint64_t frame_id, vector<uint64_t>comp_ids, string data){
  require_auth(get_self() );
...
ACTION fdacdaocore1::fileupload(name uploader, name file_scope, string content){
...
ACTION fdacdaocore1::filepublish(name file_scope, string title, checksum256 trx_id, uint32_t block_num){
  require_auth(get_self() );
  check(file_scope.value != 0, "must supply a non empty file_scope");
  checksum256 test;
...
ACTION fdacdaocore1::filedelete(name file_scope, uint64_t id){
  require_auth(get_self() );
  dacfiles_table _dacfiles(get_self(), file_scope.value);
...
void fdacdaocore1::on_transfer(name from, name to, asset quantity, string memo){
...
ACTION fdacdaocore1::clearbals(name scope){
  require_auth(get_self());
